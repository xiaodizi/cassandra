 ...
 * Since the tool mainly exercises the {@link io.github.xiaodizi.db.compaction.Verifier}, we elect to
 * not run every conceivable option as many tests are already covered by {@link io.github.xiaodizi.db.VerifyTest}.
 * 
 * Note: the complete coverage is composed of:
 * - {@link StandaloneVerifierOnSSTablesTest}
 * - {@link StandaloneVerifierTest}
 * - {@link io.github.xiaodizi.db.VerifyTest}
 */
public class StandaloneVerifierOnSSTablesTest extends OfflineToolUtils
{
    ...
    public static void setup()
    {
        ...
        System.setProperty("cassandra.partitioner", "io.github.xiaodizi.dht.ByteOrderedPartitioner");
    ...
    public void testCheckVersionWithWrongVersion() throws Exception
    {
        ...
        createAndPopulateTable(keyspace, tableName, cfs -> {
            ...
            for (io.github.xiaodizi.io.util.File cfsDir : cfs.getDirectories().getCFDirectories())
            {
    ...
    private static void populateTable(ColumnFamilyStore cfs, int partitionsPerSSTable)
    {
        ...
        io.github.xiaodizi.Util.flush(cfs);
    }
}
